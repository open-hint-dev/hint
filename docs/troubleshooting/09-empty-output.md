...
1. **Path resolved against the wrong base.** Relative CLI paths are resolved against the **current working directory**, like git and grep. Re-run with a path relative to the directory where you invoked `hint`; the error prints the attempted path (**safe to autofix**).

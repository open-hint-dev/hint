...
Warning, from `hint apply` (the command continues, but the block written to `AGENTS.md`/`CLAUDE.md` is missing that hintbook's tag glossary):
...
If the warning came from `hint apply`, re-run `hint apply` after fixing.

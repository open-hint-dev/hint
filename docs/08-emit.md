...
A pack may also ship `__imports__.tmpl`, rendered once per artifact rather than per block, whose only placeholder is `{names}`. See [Imports](#imports).
...
**Where your own code goes.** Imports above the region, helpers below it, implementations inside a hole. All three survive re-emission; nothing else between the markers does. The markers name all three zones, so the split is a structure rather than something you have to remember.

### Imports

The emitter knows the type names a spec used. It can never know which module of *this* project provides them, so it says what it knows and leaves the resolution alone:
...
```text
emit/typescript/__imports__.tmpl
──────────────────────────────────────────────
// Needs importing above this region — this project decides from where:
//   {names sep="\n//   "}
```
